...
export function Sidebar() {
    ...
    const dashboardHref = role === "Admin" ? "/admin/dashboard" : role === "Driver" ? "/driver/dashboard" : "/dashboard/dispatcher";
    const operationsSection: NavSection = {
        label: "Operations",
        items: operationsItems.map((item, index) => (index === 0 ? { ...item, href: dashboardHref } : item)),
    };

    const sections = role === "Driver"
        ? [{ label: "Operations", items: operationsSection.items.filter((item) => [dashboardHref, "/deliveries", "/tracking"].includes(item.href)) }]
        : [operationsSection, fleetSection];
