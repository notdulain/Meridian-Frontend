...
This frontend communicates directly with the Meridian backend through the **API Gateway**. Set `NEXT_PUBLIC_API_BASE_URL` in `.env.local` for local development and in the Azure Static Web Apps environment configuration for production deployments. For the current production Azure deployment, this should point to `https://ca-api-gateway.mangotree-31a6d605.eastasia.azurecontainerapps.io`.
